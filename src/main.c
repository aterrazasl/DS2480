...
int main(void)
{
	...
	system("stty -F /dev/ttyUSB0 9600 -icanon min 1 time 1 -parenb -cstopb cs8 -crtscts cread clocal -ixon -ixoff -ixany -echo -echoe -isig -opost");
	...
	readROMcmd(fd,ROMaddress);


	unsigned int count = 10;
	while(count){
		startTempConv(fd,ROMaddress);
		readScratchPAD(fd, ROMaddress, padData);
